...
def register_routes(app):
    ...
    async def fetch_robot_list():
        """获取格式化的机器人列表及其状态"""
        # 获取所有设备
        device_list_response = await YunjiService.get_device_list()
    ...
    async def fetch_device_status(device_id):
        """获取单个设备的状态"""
        device_status = await YunjiService.get_device_status(device_id)
    ...
    async def fetch_device_task(device_id):
        """获取单个设备的任务"""
        device_task = await YunjiService.get_device_task(device_id)
    ...
    async def fetch_cabin_position(device_id):
        """获取机柜位置"""
        cabin_position = await YunjiService.get_cabin_position(device_id)
    ...
    async def reset_cabin_position(device_id, position):
        """重置机柜位置"""
        result = await YunjiService.reset_cabin_position(device_id, position)
...
async def process_robot_devices(device_list):
    ...
    for robot in robot_devices:
        ...
        # 获取机器人状态
        device_status_response = await YunjiService.get_device_status(robot_id)
